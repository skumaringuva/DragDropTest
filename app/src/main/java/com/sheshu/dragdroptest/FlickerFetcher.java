...
import com.sheshu.dragdroptest.MainActivity.UIHandler;
...
class FlickerFetcher {
    ...
    private static final String APIKEY_SEARCH_STRING = "&api_key=8ad93e2cb4bfa16229941df5534226ee";
    ...
    public static void getImageURLS(FlickerItem imgCon) {
        String url = createURL(FLICKR_GET_SIZES_ID, imgCon.id);
        ByteArrayOutputStream baos = Utils.readBytes(url);
    ...
    public static Bitmap getImage(FlickerItem imgCon) {
        Bitmap bm = null;
        try {
            ...
            Log.e("FlickerFetcher", e.getMessage());
        }
        return bm;
    }

    public static void getThumbnails(ArrayList<FlickerItem> imgCon, UIHandler uih) {
        for (int i = 0; i < imgCon.size(); i++)
            new GetThumbnailsThread(uih, imgCon.get(i)).start();
    }

    static Bitmap getThumbnail(FlickerItem imgCon) {
        Bitmap bm = null;
        try {
    ...
    static ArrayList<FlickerItem> searchImagesByTag(UIHandler uih, Context ctx, String tag) {
        uihandler = uih;
        String url = createURL(FLICKR_PHOTOS_SEARCH_ID, tag);
        ArrayList<FlickerItem> tmp = new ArrayList<FlickerItem>();
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < imageJSONArray.length(); i++) {
                    ...
                    FlickerItem imgCon = new FlickerItem(item.getString("id"), item.getString("owner"), item.getString("secret"), item.getString("server"),
                            item.getString("farm"));
                    imgCon.position = i;
                    tmp.add(imgCon);
                }
    ...
    static class GetThumbnailsThread extends Thread {
        final UIHandler uih;
        final FlickerItem imgContener;

        GetThumbnailsThread(UIHandler uih, FlickerItem imgCon) {
            this.uih = uih;
